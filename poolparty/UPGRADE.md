...
Graph Modeling (PoolParty) 10.2 migrates to Elasticsearch 9.x, which requires a mandatory migration for old deployments
...
Starting with Graph Modeling 10.2.0 (Helm chart 0.3.x), each instance can be configured with its own Elasticsearch index
prefix and/or suffix. This is required when multiple Graph Modeling instances share a single Elasticsearch cluster.
Without unique names, all instances would read and write the same indices.

#### New Deployments

For a brand-new deployment that will share an Elasticsearch cluster with other Graph Modeling instances, set a prefix
...
On first startup Graph Modeling will create its indices using the configured names (e.g. `instance1-conceptdata`,
...
1. **Stop the running Graph Modeling instance.**
...
3. **Upgrade the Helm release:**
   ...
   ```shell
   helm upgrade graph-modeling graph-modeling-charts/graph-modeling -f your-values.yaml
   ```

4. **Start Graph Modeling.** On the first startup with the new configuration, Graph Modeling automatically creates
   Elasticsearch **aliases** that map the new prefixed index names to the existing underlying indices.  
   For example:
...
> [!NOTE]
> The alias is only created automatically, if the original index has no existing aliases. If another Graph Modeling
> instance already holds an alias on a given index, this instance will create a fresh, empty index under the prefixed
> name instead.
...
Graph Modeling will create fresh indices for it because the legacy index already has aliases from the first instance.
