...
- Updates Graph Modeling (PoolParty) to **10.3.2**. The version contains few improvements, bug fixes and new features.
...
- Updates the [examples](examples) to use `ontotext/poolparty-elasticsearch:9.4.5`. The new version of Elasticsearch
  contains security patches.
- Updates the Keycloak dependency example to use the latest image `ontotext/poolparty-keycloak:2.6.1`. The new version
  contains:
