...
![AppVersion: 10.3.1](https://img.shields.io/badge/AppVersion-10.3.1-informational?style=flat-square)
...
| Helm chart version | PoolParty version | Notes                      |
|--------------------|-------------------|----------------------------|
| 0.1.x              | 10.0.x            |                            |
| 0.2.x              | 10.1.x            |                            |
| 0.3.x              | 10.2.x            | ⚠️ Elasticsearch migration |
| 1.0.x              | 10.3.x            | ⚠️ Keycloak migration      |
...
# Alternatively - https://keycloak.[your ip address].nip.io/auth
export KEYCLOAK_URL="http://$KEYCLOAK_FQDN/auth"

# Alternatively - https://graphdb.[your ip address].nip.io
export GRAPHDB_URL="http://graphdb.[your ip address].nip.io"

# Replace the Keycloak and GraphDB addresses and apply the keycloak.yaml
sed -e "s/\[KEYCLOAK_FQDN\]/$KEYCLOAK_FQDN/" \
    -e "s/\[KEYCLOAK_URL\]/$KEYCLOAK_URL/" \
    -e "s/\[GRAPHDB_URL\]/$GRAPHDB_URL/" examples/dependencies/keycloak.yaml | \
...
3. Install PoolParty
   ...
   Use [default-properties/values.yaml](examples/default-properties/values.yaml) to create `values_overrides.yaml` and
   adjust the properties. Afterward, it can be used to install the PoolParty.

    ```shell
    helm install poolparty --values values_overrides.yaml poolparty-semantic-suite/poolparty
    ```
...
| labels | object | `{}` |  |
| license.existingSecret | string | `"poolparty-license"` |  |
