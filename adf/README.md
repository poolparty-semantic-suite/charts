...
This Helm chart makes it easy to deploy and manage ADF on your [Kubernetes](https://kubernetes.io/) cluster.

# About ADF

The Graphwise Application Development Framework (ADF) is a low-code development framework for the Graphwise Platform.
The framework allows you to easily create and train semantic AI applications.
...
ADF requires running Graph Modeling, which it extend and uses as basis. For more details about Graph Modeling
installation, please check the `/poolparty` directory and the [README.md](../poolparty/README.md) file there. # TODO
...
configuration:
  properties:
    ...
    POOLPARTY_URL: "http://graph-modeling.example.com"
```
...
By default, ADF's Helm chart comes with a default Ingress. The Ingress can be disabled by switching `ingress.enabled` to
`false`.
...
| configuration.externalUrl | string | `"http://graph-modeling.127.0.0.1.nip.io/ADF"` |  |
...
When expanding your Minikube cluster from one to two or more nodes to deploy different ADF instances across multiple
nodes, you may encounter errors when setting up persistent storage. These issues are due to implementation problems with
the storage provisioner included with Minikube. To resolve this, you need to adjust your environment accordingly. Follow
the steps outlined in the official Minikube documentation under the
