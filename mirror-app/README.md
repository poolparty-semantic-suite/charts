...
[Mirror App](https://help.graphwise.ai/en/graph-automation/how-to-mirror-a-project-with-the-mirror-app.html)!
This Helm chart makes it easy to deploy and manage Mirror App on your [Kubernetes](https://kubernetes.io/) cluster.

# About Mirror App

The Mirror App is used for a one-way synchronization (mirroring) of a project from a source Graph Modeling server to one
or more target Graph Modeling servers. The existing content of the target project will be deleted and replaced with an
exact copy of the source project. The synchronization does not include ontologies and custom schemes, but they can be
updated manually.
...
| configuration.existingServerMap.secret | string | `""` |  |
| configuration.existingServerMap.secretKey | string | `"Server-map.json"` |  |
| configuration.externalUrl | string | `"http://poolparty.127.0.0.1.nip.io/PoolParty/mirror"` |  |
