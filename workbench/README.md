...
Welcome to the official [Helm](https://helm.sh/) chart repository for Semantic Workbench!
...
The Workbench supports subject matter experts in finding the optimal combination of parameters for inference tagging and
semantic recommender features. The Workbench is a useful tool allowing subject matter experts to define specific
settings for the required calls (which can be extraction, expansion and recommendation); these calls are stored together
in a configuration in a DB and can be loaded to be edited or tested at any point using the Workbench.

[Learn more](https://help.graphwise.ai/en/graphwise-platform-overview/graphwise-sub-components-at-a-glance/semantic-workbench-at-a-glance.html)
...
Semantic Workbench requires running Graph Modeling, to which it will connect and basically extend. For more details
about Graph Modeling installation, please check the `/poolparty` directory and the [README.md](../poolparty/README.md) # TODO
file there.
...
3. Install Semantic Workbench
    ...
    ```shell
    helm install semantic-workbench graph-modeling-charts/workbench
    ```
...
Most configuration properties have default values, but as shown in the example below, the URLs to the Graph Modeling
dependencies must be provided, along with the name of the secret containing the license.
...
configuration:
  properties:
    POOLPARTY_URL: http://graph-modeling:8081
...
```shell
helm install semantic-workbench graph-modeling-charts/workbench -f values_overrides.yaml
```
...
| configuration.externalUrl | string | `"http://graph-modeling.127.0.0.1.nip.io/SemanticWorkbench"` |  |
...
| configuration.properties.KEYCLOAK_URL | string | `"http://keycloak.127.0.0.1.nip.io/auth"` |  |
| configuration.properties.POOLPARTY_URL | string | `"http://graph-modeling.127.0.0.1.nip.io"` |  |
...
across multiple nodes, you may encounter errors when setting up persistent storage. These issues are due to
implementation problems with the storage provisioner included with Minikube. To resolve this, you need to adjust your
environment accordingly. Follow the steps outlined in the official Minikube documentation under the
