...
[GraphViews](https://help.graphwise.ai/en/graphwise-platform-overview/graphwise-sub-components-at-a-glance/graphwise-graphviews.html)!
This Helm chart makes it easy to deploy and manage GraphViews on your [Kubernetes](https://kubernetes.io/) cluster.

# About GraphViews

Graphwise GraphViews makes it easy to gain an overview of knowledge graphs. It allows users to explore, visualize, and
interact with linked data to uncover connections, identify trends, exceptions, and patterns, and gain new insights.
Graph visualizations support informed decision-making and can be downloaded or shared to integrate seamlessly into
analysts workflows.
...
The configurations are provided as environment variables to the application, when it is started. You must have the Graph
Modeling connection configuration in order to start up GraphViews.

```yaml
configuration:
  properties:
   PP_URL: http://graph-modeling:8081
```

The Graph Modeling user credentials should be provided via a secret:

```yaml
apiVersion: v1
kind: Secret
metadata:
  name: graoph-modeling-connection-credentials
stringData:
  PP_USERNAME: "user"
...
```yaml
configuration:
  existingSecret: graph-modeling-connection-credentials
```
...
| configuration.properties.PP_URL | string | `"http://graph-modeling.127.0.0.1.nip.io"` |  |
